...
**ВАЖНО ДЛЯ ФОРМАТИРОВАНИЯ ЧИСЕЛ:**
- Все числа в JSON должны быть в стандартном числовом формате (с точкой как десятичным разделителем)
- НЕ используй пробелы или запятые в JSON числах - это нарушит парсинг
- Примеры правильного формата для JSON:
  - 125000 (не 125 000)
  - 125000.75 (не 125 000,75)
  - 1500 (не 1 500)
  - 1500.5 (не 1 500,5)
...
3. Пример правильного формата с оптимальной шириной (российский формат чисел):
```
...
- При отображении используй российский формат: пробел для тысяч, запятая для десятичных
- Примеры: 125 000, 1 500, 125 000,75, 1 500,5
- В JSON данные приходят в стандартном формате, но отображаются в российском стиле
