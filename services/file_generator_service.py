...
class FileGeneratorService:
    ...
    def generate_excel_file(self, receipt_data: ReceiptData, matching_result: IngredientMatchingResult) -> Optional[str]:
        """
        Generate Excel file with the same data format as Google Sheets upload (synchronous version for compatibility)
    ...
    async def generate_excel_file_async(self, receipt_data: ReceiptData, matching_result: IngredientMatchingResult) -> Optional[str]:
        """
        Generate Excel file with the same data format as Google Sheets upload (async version)
        ...
        try:
            ...
            # Write to Excel file without headers (this is still synchronous as pandas doesn't support async)
            with pd.ExcelWriter(filepath, engine='openpyxl') as writer:
                df.to_excel(writer, sheet_name='Receipt Data', index=False, header=False)
